...
(function($ , undefined) {
	...
	Ace_File_Input.prototype.show_file_list = function($files , inner_call) {
		...
		for (var i = 0; i < files.length; i++) {
			...
			var fileIcons = {
				'file' : 'fas fa-file',
				'image' : 'fas fa-picture-o file-image',
				'video' : 'fas fa-film file-video',
				'audio' : 'fas fa-music file-audio'
			};
	...
	Ace_File_Input.prototype.loading = function(is_loading) {
		if(is_loading === false) {
			...
			var inside = typeof is_loading === 'string' ? is_loading : '<i class="overlay-content fas fa-spin fa-spinner orange2 fa-2x"></i>';
	...
	$.fn.ace_wysiwyg = function($options , undefined) {
		...
		var button_defaults =
		{
			...
			'insertImage' : {
				...
				button_text : '<i class="'+ ace.vars['icon'] + 'fas fa-file"></i> Choose Image &hellip;',
		...
		this.each(function() {
			...
			for(var tb in toolbar_buttons) if(toolbar_buttons.hasOwnProperty(tb)) {
				...
				switch(button.name) {
					case 'font':
						toolbar += ' <a class="btn btn-sm '+className+' dropdown-toggle" data-toggle="dropdown" title="'+button.title+'"><i class="'+ ace.vars['icon'] + button.icon+'"></i><i class="' + ace.vars['icon'] + 'fas fa-angle-down icon-on-right"></i></a> ';
...
(function($ , undefined) {
	...
	$.fn.ace_spinner.defaults = {
		'icon_up' : 'fas fa-chevron-up',
		'icon_down': 'fas fa-chevron-down',
...
(function($ , undefined) {
	...
	$.fn.aceTree = $.fn.ace_tree = function(options) {
		var $defaults = {
			'open-icon' : ace.vars['icon'] + 'fas fa-folder-open',
			'close-icon' : ace.vars['icon'] + 'fas fa-folder',
			'selectable' : true,
			'selected-icon' : ace.vars['icon'] + 'fas fa-check',
			'unselected-icon' : ace.vars['icon'] + 'fas fa-times',
			'loadingHTML': 'Loading...'
		}
