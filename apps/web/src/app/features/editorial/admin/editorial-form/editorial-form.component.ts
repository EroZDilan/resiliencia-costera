...
@Component({
  ...
  template: `
    <p-toast />
    <h2>{{ id ? 'Editar editorial' : 'Nueva editorial' }}</h2>
    <form [formGroup]="form" (ngSubmit)="submit()" class="admin-form">
      ...
      <div class="admin-form-actions">
        <button pButton type="button" label="Cancelar" class="p-button-text" routerLink="/admin/editoriales"></button>
...
export class EditorialFormComponent implements OnInit {
  ...
  submit(): void {
    ...
    request$.subscribe({
      next: () => this.router.navigate(['/admin/editoriales']),
      error: (err) => {
        this.saving = false;
