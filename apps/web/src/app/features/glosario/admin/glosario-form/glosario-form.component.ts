...
@Component({
  selector: 'app-glosario-form',
  standalone: true,
  imports: [ReactiveFormsModule, RouterLink, ButtonModule, InputTextModule, TextareaModule, ToastModule],
  providers: [MessageService],
  template: `
    <p-toast />
    <h2>{{ id ? 'Editar término' : 'Nuevo término' }}</h2>
    <form [formGroup]="form" (ngSubmit)="submit()" class="admin-form">
      <div class="admin-field">
        <label for="termino">Término *</label>
        <input pInputText id="termino" formControlName="termino" />
      </div>
      <div class="admin-field">
        <label for="significado">Significado *</label>
        <textarea pTextarea id="significado" formControlName="significado" rows="5"></textarea>
      </div>
      <div class="admin-form-actions">
        <button pButton type="button" label="Cancelar" class="p-button-text" routerLink="/admin/glosario"></button>
        <button pButton type="submit" label="Guardar" [loading]="saving"></button>
      </div>
    </form>
  `,
})
export class GlosarioFormComponent implements OnInit {
  ...
  submit(): void {
    ...
    request$.subscribe({
      next: () => this.router.navigate(['/admin/glosario']),
      error: (err) => {
        this.saving = false;
