...
@Component({
  ...
  template: `
    <p-toast />
    <h2>{{ id ? 'Editar colaboracion' : 'Nueva colaboracion' }}</h2>
    <form [formGroup]="form" (ngSubmit)="submit()" class="admin-form">
      ...
      <div class="admin-field">
        ...
        <textarea pTextarea id="textoCompleto" formControlName="textoCompleto" rows="8"></textarea>
      </div>
      <div class="admin-field">
        <label>Adjunto (PDF) *</label>
        @if (adjunto?.archivo) { <span>{{ adjunto?.archivo }}</span> }
        <p-fileUpload mode="basic" chooseLabel="Subir adjunto" accept="application/pdf"
      ...
      <div class="admin-form-actions">
        <button pButton type="button" label="Cancelar" class="p-button-text" routerLink="/admin/colaboraciones"></button>
...
export class ColaboracionFormComponent implements OnInit {
  ...
  form = this.fb.nonNullable.group({
    ...
    textoCompleto: ['', Validators.required],
  });
  ...
  submit(): void {
    if (this.form.invalid || !this.adjunto) {
      this.toast.add({ severity: 'warn', summary: 'Faltan datos', detail: !this.adjunto ? 'Debe adjuntar un PDF.' : 'Complete los campos requeridos.' });
      return;
    }
    this.saving = true;
