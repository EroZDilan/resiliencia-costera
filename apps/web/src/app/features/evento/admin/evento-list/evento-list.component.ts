...
@Component({
  ...
  template: `
    ...
    <div class="admin-header">
      <h2>Eventos</h2>
      <button pButton label="Nuevo" icon="pi pi-plus" routerLink="/admin/eventos/nuevo"></button>
    </div>
    <p-table [value]="items" [loading]="loading" [paginator]="true" [rows]="20" dataKey="id">
      ...
      <ng-template pTemplate="body" let-item>
        <tr>
          <td>{{ item.titulo }}</td>
          <td>{{ item.fechaInicio | date: 'short' }} — {{ item.fechaFin | date: 'short' }}</td>
          <td class="admin-actions">
            <button pButton icon="pi pi-pencil" class="p-button-text" [routerLink]="['/admin/eventos', item.id]"></button>
...
export class EventoListComponent implements OnInit {
  ...
  confirmRemove(item: Evento): void {
