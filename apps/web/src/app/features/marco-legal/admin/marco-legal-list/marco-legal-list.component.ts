...
@Component({
  ...
  template: `
    ...
    <div class="admin-header">
      <h2>Marco legal</h2>
      <button pButton label="Nuevo" icon="pi pi-plus" routerLink="/admin/marco-legal/nuevo"></button>
    </div>
    <p-table [value]="items" [loading]="loading" [paginator]="true" [rows]="20" dataKey="id">
      <ng-template pTemplate="header">
        <tr><th>Título</th><th>Tipo</th><th>Emisor</th><th>Año</th><th></th></tr>
      </ng-template>
      <ng-template pTemplate="body" let-item>
        <tr>
          ...
          <td class="admin-actions">
            <button pButton icon="pi pi-pencil" class="p-button-text" [routerLink]="['/admin/marco-legal', item.id]"></button>
...
export class MarcoLegalListComponent implements OnInit {
  ...
  confirmRemove(item: MarcoLegal): void {
    this.confirm.confirm({
      ...
      accept: () => {
        this.service.remove(item.id).subscribe(() => {
          this.toast.add({ severity: 'success', summary: 'Eliminado', detail: item.titulo });
          this.load();
        });
      },
    });
